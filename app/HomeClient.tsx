...
const safeNameRegex = /[^\p{L}\p{N}\s._-]/gu;
...
function sanitizeName(value: string): string {
  return value.replace(safeNameRegex, "").trim().slice(0, maxNameLength);
}
...
export default function HomeClient({ initialFrom = "" }: HomeClientProps) {
  const fromParam = sanitizeName(initialFrom);
  ...
  const createLink = () => {
    const trimmed = sanitizeName(nameInput);
    if (!trimmed) return;
